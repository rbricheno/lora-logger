...
type LoraLoggerTestSuite struct {
	suite.Suite
...
func (ts *LoraLoggerTestSuite) SetupTest() {
...
func (ts *LoraLoggerTestSuite) TearDownTest() {
	assert := require.New(ts.T())
...
func (ts *LoraLoggerTestSuite) TestUplink() {
	tests := []struct {
		Name                    string
...
func (ts *LoraLoggerTestSuite) TestDownlinkBackend1() {
	tests := []struct {
		Name                    string
		BackendSent             []byte
...
func TestLoraLogger(t *testing.T) {
	suite.Run(t, new(LoraLoggerTestSuite))
}
